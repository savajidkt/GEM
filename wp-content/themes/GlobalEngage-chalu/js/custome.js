...
//   var customSelectEle, i, j, selElmnt, divEle, divEleSelected, c;
//   customSelectEle = document.querySelector(".drop-item");
//   selElmnt = customSelectEle.getElementsByTagName("select")[0];
//   divEle = document.createElement("DIV");
//   divEle.setAttribute("class", "select-selected");
//   divEle.innerHTML = selElmnt.options[selElmnt.selectedIndex].innerHTML;
...
jQuery(function ($) {
    jQuery(document).ready(function () {
        ...
        let allpopup = document.querySelectorAll('#main_img_popup');
for (var i = 0; i < allpopup.length; i++) {
    console.log(allpopup[i].style.display);
    if(allpopup[i].style.display=="block"){
        body.classList.add("harry");
    }else{
         body.classList.remove("harry");
    }
    //Do something
}
