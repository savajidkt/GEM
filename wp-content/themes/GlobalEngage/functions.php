...
function so_29984159_custom_content_template_part( $template, $slug, $name ){
    ...
    $id = $post->ID;
        ...
        if ( $slug == 'content' && $name == 'single-product')  {
            ...
            if(get_field('event_type') == 'conference'){
             $template = dirname( __FILE__ ) . '/woocommerce/single-conference.php';
            }
...
{
    if ('first-name' === $tag_data['name'] && is_user_logged_in()) {
        $current_user = wp_get_current_user();
        $tag_data['values'] = array($current_user->display_name);
    }

    if ('last-name' === $tag_data['name'] && is_user_logged_in()) {
        $current_user = wp_get_current_user();
...
function wpb_posts_nav(){
    $next_post = get_next_post(true,'','resource-type');
    $prev_post = get_previous_post(true,'','resource-type');
...
function getContactFormData($form)
{
    ...
    if (is_user_logged_in()) {
        ...
        global $wpdb;
        $qry = "SELECT * FROM wp_db7_forms where form_post_id ='$id' ";
        $contactData = $wpdb->get_results($qry);
       
        if (is_array($contactData) && count($contactData) > 0) {
            foreach ($contactData as $key => $value) {
                $unserData = unserialize($value->form_value);
                if (is_array($unserData) && count($unserData) > 0) {

                    if ($form === 'form1') {
                        if ($unserData['your-email'] == wp_get_current_user()->user_email) {
                            return true;
                        }
                    } elseif ($form === 'form2') {
                        if ($unserData['email-425'] == wp_get_current_user()->user_email) {
                            return true;
                        }
                    }
                }
...
function product_type_selector_filter_callback() {
    ...
    <script>
    jQuery(function($){
        jQuery(document).on("change", "select#product-type", function(e){
            var product_type = $(this).val();
            if(product_type == 'variable'){
                jQuery(".product_data_tabs li").each(function() {
                    $(this).removeClass('active');
                  });
                $('.general_options').removeClass('hide_if_grouped');
...
function dateFormatChange($discountDate){
   echo date('j',strtotime($discountDate)).'<sup class="date-sub"> '.date('S',strtotime($discountDate)).'</sup> '.date(' F Y',strtotime($discountDate));
}

function dateFormatChangeCart($discountDate){
  return date('j',strtotime($discountDate)).'<sup class="date-sub"> '.date('S',strtotime($discountDate)).'</sup> '.date(' F Y',strtotime($discountDate));
}
...
function cart_item_product_description( $item_name, $cart_item, $cart_item_key ) {
    ...
    if($_product->parent_id > 0){
        $product = wc_get_product($_product->parent_id);
     $eventDate = $product->get_meta('WooCommerceEventsDate',true);
    }else{
        $eventDate = $_product->get_meta('WooCommerceEventsDate',true);
    }

    if (!is_checkout() ) {
        if( $cart_item['variation_id'] > 0 ) {
        $item_name.= '<br><div class="description">
               <strong>' . __( 'Delated Package', 'woocommerce' ) . '</strong></div>';   

        }

        // if ( ! empty($eventDate) ) {
        //     return $item_name . '<br><div class="description">
        //         <strong>' . __( 'Delated Package:', 'woocommerce' ) . '</strong>: '. $description . '
        //     </div>';
        // }
        if ( ! empty($eventDate) ) {
            return $item_name . '<br><div class="description">
                <strong>' . __( 'Date', 'woocommerce' ) . '</strong>: '. dateFormatChangeCart($eventDate) . '
            </div>';
        }
    }
    return $item_name;
