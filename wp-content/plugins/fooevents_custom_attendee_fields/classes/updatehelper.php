...
class Fooevents_Custom_Attendee_Fields_Update_Helper {
	...
	public function __construct( $config ) {
		...
		add_filter( 'plugins_api', array( $this, 'plugin_update_information' ), 22, 3 );
		add_action( 'in_plugin_update_message-fooevents_custom_attendee_fields/fooevents-custom-attendee-fields.php', array( $this, 'show_upgrade_notification' ), 10, 2 );
	...
	public function plugin_update_information( $res, $action, $args ) {
		...
		$plugin_slug = 'fooevents_custom_attendee_fields/fooevents-custom-attendee-fields.php';
		...
		if ( false === $remote = get_transient( 'fooevents_update_' . $plugin_slug ) ) {
			...
			$remote = wp_remote_get(
				'https://www.fooevents.com/update_info/fooevents_custom_attendee_fields.json',
				array(
					'timeout' => 10,
					'headers' => array(
	...
	private function get_latest_plugin_details_fooevents() {
		...
		$last_update_check    = get_option( '_fooevents_attendees_last_update_check', true );
		$last_update_response = get_option( '_fooevents_attendees_last_update_response', true );
		...
		if ( empty( $last_update_check ) || 1 === (int) $last_update_check || $last_update_check <= $expire_check ) {
			...
			if ( ! is_wp_error( $response ) ) {
				...
				update_option( '_fooevents_attendees_last_update_response', $response );
			...
			update_option( '_fooevents_attendees_last_update_check', $timestamp );
	...
	public function show_upgrade_notification( $current_plugin_meta_data, $new_plugin_meta_data ) {
		...
		if ( ! empty( $this->fooevents_reponse ) ) {
			...
			if ( $this->fooevents_reponse['status'] == 'error' ) {
	...
	public function after_plugin_update() {
		...
		update_option( '_fooevents_attendees_last_update_check', '' );
		update_option( '_fooevents_attendees_last_update_response', '' );
