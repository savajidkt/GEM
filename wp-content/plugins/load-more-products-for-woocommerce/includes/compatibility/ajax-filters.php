...
class BeRocket_LMP_compat_ajax_filter {
    ...
    public static function aapf_localize_widget_script($localize) {
        $new = is_array($localize['woocommerce_removes']);
        if(! $new) {$localize['woocommerce_removes'] = json_decode($localize['woocommerce_removes'], true);}
        $localize['woocommerce_removes']['pagination'] = '';
        if(! $new) {$localize['woocommerce_removes'] = json_encode($localize['woocommerce_removes']);}
        return $localize;
    }
