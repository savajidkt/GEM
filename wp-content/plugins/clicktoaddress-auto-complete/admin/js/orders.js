...
window.cc_c2a_functions.autocomplete.add_autocomplete = function() {
			...
			if (!window.cc_c2a_config.autocomplete.search_line_1) {
				if (window.cc_c2a_config.autocomplete.hide_fields) {
					...
					if (typeof elements === 'undefined') {
						jQuery('div.edit_address').each(function(index, elem) {
	...
	setInterval(function() {
		...
		if (jQuery('#_billing_postcode').length && jQuery('#_billing_address_1').attr('cc-applied') != "1") {
			window.cc_c2a_functions.autocomplete.add_autocomplete_form('_billing');
		}
		// Add lookup to shipping form
		if (jQuery('#_shipping_postcode').length && jQuery('#_shipping_address_1').attr('cc-applied') != "1") {
			window.cc_c2a_functions.autocomplete.add_autocomplete_form('_shipping');
		}
	}, 200);
}
...
window.cc_c2a_functions.autocomplete.add_autocomplete_form = function(prefix) {
	...
	if (!window.cc_c2a_config.autocomplete.search_line_1) {
		...
		var tmp_html = jQuery(	'<p class="form-field ' + prefix + '_cc_c2a_search_input_field">' +
									'<label id="fetchify_find_label_' + prefix + '" for="' + prefix + '_cc_c2a_search_input">' +
		...
		// Move country field above line 1
		jQuery(dom.country).parents('p').insertBefore(jQuery(dom.line_1).parents('p'));
		...
		jQuery(dom.country).parents('p').css('width', '100%');
	}
	...
	if (window.cc_c2a_config.autocomplete.hide_fields && !window.cc_c2a_config.autocomplete.search_line_1) {
		...
		var fetchify_label = jQuery('#fetchify_find_label_' + prefix);
		cc_manual_button.insertBefore(fetchify_label);
...
window.cc_c2a_functions.email.add_email = function() {
	...
	var cc_email_interval = setInterval(function() {
		// Add lookup to billing form
		if (jQuery('#_billing_email').length && jQuery('#_billing_email').attr('cc-applied') != "1") {
			window.cc_c2a.addEmailVerify({
				email: '#_billing_email'
			});
			jQuery('#_billing_email').attr('cc-applied', "1");
...
window.cc_c2a_functions.postcode.add_postcode = function() {
	...
	if (window.cc_c2a_config.postcode.enabled && !window.cc_c2a_config.autocomplete.enabled) {
		jQuery('#_billing_country').on('change', function(){
			window.cc_c2a_functions.postcode.add_postcode_form('_billing');
		});
		jQuery('#_shipping_country').on('change', function(){
			window.cc_c2a_functions.postcode.add_postcode_form('_shipping');
		});
	}
}
	...
	if (craftyCountry == 'GB' || craftyCountry == 'JE' || craftyCountry == 'GG' || craftyCountry == 'IM') {
		if (!jQuery('#' + prefix + '_cp_button').length) {
			...
			cp_obj.set("on_result_ready", function() {
				...
				window.cc_c2a_functions.postcode.setup_for_uk();
		...
		if (jQuery('#' + prefix + '_address_1').val() == '' && jQuery('#crafty_postcode_result_display_' + prefix).is(':empty')) {
			setTimeout(function() {
...
window.cc_c2a_functions.postcode.setup_for_uk = function() {
