...
window.cc_c2a_functions.autocomplete.add_autocomplete = function() {
	...
	var c2a_autocomplete_config = {
		...
		domMode:					'object',
		gfxMode:					window.cc_c2a_config.autocomplete.layout,
		style: {
			ambient:				window.cc_c2a_config.autocomplete.ambient,
			accent:					window.cc_c2a_config.autocomplete.accent
		},
		getIpLocation:				window.cc_c2a_config.autocomplete.ip_location,
		...
		onResultSelected: function(c2a, elements, address){
			...
			var elems_to_update = ['company', 'country', 'county', 'line_1', 'line_2',  'postcode', 'town'];
			for (var i = 0; i < elems_to_update.length; i++) {
				if (jQuery(elements[elems_to_update[i]]).length) {
					jQuery(elements[elems_to_update[i]]).trigger('update').trigger('change');
				}
			}
		},
			...
			if (!window.cc_c2a_config.autocomplete.search_line_1) {
				if (window.cc_c2a_config.autocomplete.hide_fields) {
					...
					if (typeof elements === 'undefined') {
						jQuery('div[class$="__field-wrapper"]').each(function(index, elem) {
	...
	if (window.cc_c2a_config.autocomplete.lock_country) {
		...
		c2a_autocomplete_config.onSearchFocus = function(c2a, dom) {
			// Clause to catch if country is only a text (Woo standard, that if there is only one allowed country then it's only a little read-only input)
			var currentCountry;
			if (dom.country.tagName === 'INPUT') {
				currentCountry = dom.country.value;
			}
			else {
				currentCountry = dom.country.options[dom.country.selectedIndex].value;
			}
...
window.cc_c2a_functions.autocomplete.add_autocomplete_form = function(prefix) {
	...
	if (!window.cc_c2a_config.autocomplete.search_line_1) {
		...
		var tmp_html = jQuery(	'<p class="form-row form-row form-row-wide address-field">' +
									'<label id="fetchify_find_label_' + prefix+'">' +
		...
		if (window.cc_c2a_config.autocomplete.lock_country) {
			...
			tmp_html.insertBefore(jQuery(dom.country).parents('p'));
		}
	}
	...
	if (window.cc_c2a_config.autocomplete.hide_fields && !window.cc_c2a_config.autocomplete.search_line_1) {
		...
		var cc_hide_button = jQuery(	'<p class="cc_c2a_hide">' +
										...
										'</p>');
		window.cc_c2a_functions.autocomplete.add_button_css(cc_manual_button);
		window.cc_c2a_functions.autocomplete.add_button_css(cc_hide_button);
		cc_manual_button.insertBefore(jQuery('#fetchify_find_label_' + prefix));
		if (window.cc_c2a_config.autocomplete.lock_country) {
			cc_hide_button.insertBefore(jQuery(dom.line_1).closest('p').find('label')[0]);
		}
		else {
			cc_hide_button.insertBefore(jQuery(dom.country).closest('p').find('label')[0]);
		}
...
window.cc_c2a_functions.autocomplete.countries_by_wc = function() {
	...
	for (var i = 0; i < country_elems.length; i++) {
		if (country_elems[i].length) {
			// If only 1 country enabled in WooCommerce and dropdown is hidden
			if (country_elems[i].attr('type') == 'hidden' && countryNames.indexOf(country_elems[i].val()) === -1) {
				countryNames.push(country_elems[i].val());
			}
...
window.cc_c2a_functions.email.add_email = function() {
	...
	var cc_email_interval = setInterval(function() {
		// Add lookup to billing form
		if (jQuery('#billing_email').length && jQuery('#billing_email').attr('cc-applied') != "1") {
			window.cc_c2a.addEmailVerify({
				email: '#billing_email'
			});
			jQuery('#billing_email').attr('cc-applied', "1");
			clearInterval(cc_email_interval);
		}
	}, 200);	
}
...
window.cc_c2a_functions.postcode.add_postcode_form = function(prefix) {
	if (jQuery('#' + prefix + '_address_1').length && typeof CraftyPostcodeCreate !== 'undefined') {
		...
		if (craftyCountry == 'GB' ||
			...
			craftyCountry == 'GG' ||
			craftyCountry == 'IM' ||
			typeof craftyCountry == 'undefined')
		{
			if (!jQuery('#' + prefix + '_cp_button').length) {
				...
				var tmp_html =
					...
					'<p class="form-row form-row-last address-field crafty_' + prefix + '" ' +
						...
						'<button class="button alt" ' +
							...
							'type="button" style="' + window.cc_c2a_config.postcode.button_css + '">' +
								window.cc_c2a_config.postcode.button_text +
						'</button>' +
					'</p>'+
					'<p class="form-row form-row-wide address-field ' + prefix + '_cp_result_class crafty_' + prefix + '" ' +
				...
				if (window.cc_c2a_config.postcode.button_css.indexOf('font-size') === -1) {
				...
				cp_obj.set("on_result_ready", function() {
					...
					window.cc_c2a_functions.postcode.rearrange_fields(false);
			...
			setTimeout(function() {
				window.cc_c2a_functions.postcode.rearrange_fields(prefix);
			}, 100);
		}
		else {
...
window.cc_c2a_functions.postcode.on_result_selected = function(prefix, suffix) {
	...
	// Call upon the update event to trigger the Almighty WooCommerce Input Validator
	// Without this, plugins which wait for the validated value will not receive our auto-filled values
	var elems_to_update = ['_company', '_country', '_state', '_address_1', '_address_2', '_postcode', '_postcode_search', '_city'];
	for (var i = 0; i < elems_to_update.length; i++) {
		if (jQuery('#' + prefix + elems_to_update[i]).length) {
			jQuery('#' + prefix + elems_to_update[i]).trigger('update').trigger('change');
		}
	}
}

window.cc_c2a_functions.postcode.rearrange_fields = function(hide) {
	...
	if (hide && jQuery('#' + hide + '_address_1').val() == '' && jQuery('#crafty_postcode_result_display_' + hide).is(':empty')) {
