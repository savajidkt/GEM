...
//		Minify email content
	...
	public function get_template_id_has_order( $type, \WC_Order $order ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( ( empty( $rule_categories ) || count( array_intersect( $categories, (array) $rule_categories ) ) ) && ( empty( $rule_products ) || count( array_intersect( $bought_ids, (array) $rule_products ) ) ) ) {
				if ( ! $max_subtotal ) {
		...
		return apply_filters( 'viwec_find_email_template_id_with_rule_order', current( $filter_ids ), $posts, $country_code, $categories, $order_subtotal );
	}
