...
class Project extends Structure\Component {
	...
	public function save_entity_by_structure( array $entity_structure ) {
		...
		if ( is_null( $entry ) ) {
			$result = new \WP_Error(
				...
				__( 'Error saving post', 'woocommerce-product-filters' )
			);
		}
		...
		if ( 'virtual' === $entity_structure['status'] ) {
			...
			if ( 0 === $result ) {
				$result = new \WP_Error( 'failure_insert_post', __( 'Error inserting post', 'woocommerce-product-filters' ) );
			} else {
			...
			if ( ! $is_virtual && get_post_status( $entity_structure['entityId'] ) !== false ) {
				if ( wp_delete_post( $entity_structure['entityId'], true ) === false ) {
					$result = new \WP_Error( 'failure_delete_post', __( 'Error deleting post', 'woocommerce-product-filters' ) );
				}
			} elseif ( ! $is_virtual ) {
				$result = new \WP_Error( 'failure_delete_post', __( 'Error deleting post', 'woocommerce-product-filters' ) );
			}
			...
			if ( get_post_status( $entity_structure['entityId'] ) !== false ) {
				...
				if ( 0 === $result ) {
					$result = new \WP_Error( 'failure_update_post', __( 'Error updating post', 'woocommerce-product-filters' ) );
				}
			} else {
				$result = new \WP_Error( 'failure_update_post', __( 'Error updating post', 'woocommerce-product-filters' ) );
			}
	...
	public function save_project_by_structure( array $project_structure ) {
		...
		try {
			...
			$result = new WP_Error( 'save_project_error', __( 'An error occurred while saving project', 'woocommerce-product-filters' ) );
		}
