...
class Text extends AbstractRenderer
{
	...
	if ( ( strtolower( $style->direction ) == 'rtl' && ! mb_detect_encoding( $text, array( 'ASCII' ) ) ) || preg_match( '/\p{Hebrew}/u', $text ) ) {
		$add_space = '';
		if ( preg_match( '/\p{Latin}/u', substr( $text, 0, 1 ) ) ) {
			$add_space = ' ';
		}
		preg_match_all( '/./us', $text, $ar );
		$text = join( '', array_reverse( $ar[0] ) ) . $add_space;

		// if there are numbers, fullstops, email addresses or English words in the string reverse it back
