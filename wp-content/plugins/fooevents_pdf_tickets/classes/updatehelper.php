 ...
 */
class FooEvents_PDF_Tickets_Update_Helper {
	 ...
	 * Notification
	 *
	 * @var array $displayed_notification
	 */
	private $displayed_notification;
	...
	public function __construct( $config ) {
		...
		$this->displayed_notification = false;

		add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'set_transitent' ) );
		add_filter( 'plugins_api', array( $this, 'plugin_update_information' ), 23, 3 );
		add_action( 'in_plugin_update_message-fooevents_pdf_tickets/fooevents-pdf-tickets.php', array( $this, 'show_upgrade_notification' ), 10, 2 );
	...
	public function plugin_update_information( $res, $action, $args ) {
		...
		$plugin_slug = 'fooevents_pdf_tickets/fooevents-pdf-tickets.php';
		...
		if ( false == $remote = get_transient( 'fooevents_update_' . $plugin_slug ) ) {
			...
			$remote = wp_remote_get(
				'https://www.fooevents.com/update_info/fooevents_pdf_tickets.json',
				array(
					'timeout' => 10,
					'headers' => array(
	...
	private function get_latest_plugin_details_fooevents() {
		...
		$last_update_check    = get_option( '_fooevents_pdftickets_last_update_check', true );
		$last_update_response = get_option( '_fooevents_pdftickets_last_update_response', true );
		...
		if ( empty( $last_update_check ) || 1 === (int) $last_update_check || $last_update_check <= $expire_check ) {
			...
			if ( ! is_wp_error( $response ) ) {
				...
				update_option( '_fooevents_pdftickets_last_update_response', $response );
			...
			update_option( '_fooevents_pdftickets_last_update_check', $timestamp );
	...
	public function show_upgrade_notification( $current_plugin_metadata, $new_plugin_metadata ) {
		...
		if ( ! empty( $this->fooevents_reponse ) ) {
			...
			if ( $this->displayed_notification === false ) {
				...
				$this->displayed_notification = true;
	...
	public function after_plugin_update() {
		...
		update_option( '_fooevents_pdftickets_last_update_check', '' );
		update_option( '_fooevents_pdftickets_last_update_response', '' );
